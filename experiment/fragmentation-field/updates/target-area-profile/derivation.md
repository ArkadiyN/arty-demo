...
flight (drag, Mott, Gurney) or lethality (ES-310 $P_{k|\text{hit}}$). It
...
| Symbol      | Meaning                            | Unit |
| ----------- | ---------------------------------- | ---- |
| $w_\perp$ | body width (shoulder-to-shoulder)  | m    |
...
| -------------------------------------- | ------------- | -------------- | ---------------- |
| Front face $A_f$ — vertical silhouette | $w_\perp,h$ | horizontal     | $\gamma < 90°$  |
| Top face $A_t$ — overhead silhouette   | $w_\perp,d$ | vertical (up)  | $\gamma > 0°$   |
...
;=; w_\perp\bigl(h\cos\gamma + d\sin\gamma\bigr)
;}
\quad (\text{eq. P1})
$$
...
$w_\perp$ as the horizontal angular extent of the target (an *azimuthal*
quantity); the new factor $h_\text{eff}(\gamma)$ is the *vertical* extent
...
| ------------------------------- | ----------------------------------------- | ----------------------------------------- |
| $\gamma = 0$ (horizontal)      | $w_\perp,h = A_f$                       | Pure frontal silhouette.                  |
| $\gamma = 90°$ (vertical)      | $w_\perp,d = A_t$                       | Pure overhead silhouette.                 |
| $h = d$ (cube)                  | $w_\perp,h,(\cos\gamma+\sin\gamma)$ | Max $A_p = \sqrt{2},w_\perp h$ at 45°. |
| $d = 0$ (infinitely thin plate) | $w_\perp,h\cos\gamma$                 | Plate cosine law.                         |

$A_p$ is bounded above by $w_\perp\sqrt{h^2 + d^2}$ at
...
polar-angle direction — i.e., $h_\text{eff}^\text{old} = s$, not
...
extent $h_\text{eff}(\gamma) < s$ (which is the rule, not the exception —
...
$$
\Omega_\text{belt}
= \int_{90°-\delta}^{90°+\delta}!!\int_0^{2\pi} \sin\Theta,d\psi,d\Theta
...
$N_\text{hits} = A N_0/(4\pi R^2)$ form for an isotropic burst — our belt
...
Substituting eq. (P4) and $A_p(\gamma) = w_\perp,h_\text{eff}(\gamma)$ from
...
$$
A_p(\gamma) = w_\perp\bigl(h\cos\gamma + d\sin\gamma\bigr),
\qquad
\sin\gamma = h_b/s,
\qquad
...
integrand $n(m),P_{k|\text{hit}}$ multiplied by a dimensionless geometry
...
the mass integrand $n(m),P_{k|\text{hit}},dm$ has units of
[frag/kg]·[-]·[kg] = [frag] and $N_\text{eff}$ must be in [hits per target].
Confirmed.
...
- **Old factor** at $\gamma = 0$: $g_\text{old} = w/(2\pi,s\cdot 2\sin\Theta,\delta)$.
- **New factor** at $\gamma = 0$: $A_p(0) = w_\perp h = A_f$, and
  $g_\text{new}(0) = A_f/(2\pi,s^2\cdot 2\sin\Theta,\delta)$.

Their ratio is

$$
\frac{g_\text{new}(0)}{g_\text{old}} ;=;
\frac{A_f}{w,s} ;=; \frac{w_\perp h}{w,s}.
$$

With $w_\perp = w = 0.5,\text{m}$ and $h = 1.7,\text{m}$ (standing) this
...
the $R_{50}$ lethal-area numbers) will not match the old model's — they will
...
$h_\text{eff}(\gamma) \to s$, i.e. the target's effective vertical extent
...
- $h_b \to 0$ ⇒ $\gamma \to 0$ ⇒ $A_p(\gamma) \to A_f = w_\perp h$.
- $\alpha = 0$, ray along $y$-axis ⇒ $\Theta = 90°$, $\sin\Theta = 1$.
- $g_\text{new} \to A_f/(2\pi,r^2 \cdot 2\delta)$.
...
| Posture  | $w_\perp$ [m] | $h$ [m] | $d$ [m] | $A_f = w_\perp h$ [m²] | $A_t = w_\perp d$ [m²] |
| -------- | --------------- | ------- | ------- | ------------------------ | ------------------------ |
...
> dimensions $(h, d)$ used in $A_p(\gamma) = w_\perp(h\cos\gamma + d\sin\gamma)$
...
| Max $A_p = w_\perp\sqrt{h^2+d^2}$ at $\gamma^\* = \arctan(d/h)$               | ✓ (Cauchy–Schwarz)                                                                                                                                                                                                                |
| $A_p$ recovers Lambert cosine law for thin plate $d\to 0$                         | ✓                                                                                                                                                                                                                                 |
| Total expected hits over the ground integrated to $\infty$ stays finite           | ✓ — convergence comes from $P_{k}$ *(rest of this cell was lost in the source — an unescaped `\|` inside the math span truncated the table row at creation; not reconstructed here, needs @modeler follow-up)* |
...
paragraph) and a calibration cost (re-anchor $R_{50}$ against TM 9-1901
