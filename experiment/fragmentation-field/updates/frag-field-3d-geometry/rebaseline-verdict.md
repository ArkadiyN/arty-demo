...
- `scoping.md:61,65-78` — Q1 Option-C rejection and the "reconciliation"
    ...
    (`scoping.md:96-97`) must be struck (done 2026-08-08: replaced with a correctly-framed side-spray magnitude anchor; **not** re-attributed to NWC).
- `scoping.md:290` (literature-audit table) — attributes "fragment-velocity
    range 1070–2740 ft/s" to the BRL Report 126 row. Void as a *range*;
    superseded 2026-08-08 by Tolch's two genuine side-spray figures
    (2,7?0 / 3,030 f/s). Does **not** belong to NWC TP 7124.
...
- `derivation.md:379` (§7 open item 2, "M48 nose vs. side velocity check") —
    ...
    sourced; rewritten 2026-08-08 as "no held source constrains the
    nose-vs-side ordering" — BRL 126 reports no nose-spray velocity, and the
    item was **not** re-attributed to NWC TP 7124.
...
**Closed 2026-08-08.** Both markers below are discharged. The citation is
repaired at every cited site — `scoping.md` §"Evidence weighing the velocity
difference" and `derivation.md` §6 "Comparison with BRL 126" — now read
Tolch's figures as **side**-spray
(perforating ≈ 835 m/s, penetrating 923 m/s), not re-attributed to NWC TP
7124 (see "downstream contamination" list above, each line marked done). The
V0 contradiction is resolved, not papered over: `derivation.md` §6 was
rewritten from the stale hand-worked 578/1578 example to the shipped
`compute_shell_zones` output (regenerated by
`checks/zone-v0-ogive-vs-cylinder.py`), which gives M1 ogive/cylinder V0 =
1100/1039 m/s and shows the ordering is drawing-dependent (it flips for the
M107, 961/1266 m/s) — neither "ogive always faster" nor "always slower" is a
general rule, so §7 item 2 now states that no held source constrains the
ordering, rather than re-asserting a wrong reconciliation. `src/arty/zones.py`
required no code change: the 578/1578 pair the finding cites never existed in
shipped code, only in this file's stale worked example.
