...
citable surface, and the unresolved cumulative velocity distribution — were
closed in `bab141a`. The section is gone and replaced by a referral; the card
states in its own words that `tolch-1938.md` is not a citable surface for any
...
FINDING\[blocking\]: the 75mm M48 registry row was rebased onto Tolch's own weight table (M_case 5755 g -> 4962 g, V0 807 -> 890 m/s, N0 3627 -> 3801; updates/shell-case-mass-basis/derivation.md sect. 4), but checks/tolch-case-mass-basis.py:38 still hardcodes M_CASE_MODEL = 5755.2 as a literal copied from count-chain-rebaseline.py block (C), so every "model / Tolch case = 1.160" and N(model M) ratio it prints is now a superseded number; both scripts should read SHELLS from arty rather than re-typing it (affects: experiment/fragmentation-field/challenges/source-data-audit/checks/tolch-case-mass-basis.py, experiment/fragmentation-field/challenges/count-gap-1938/checks/count-chain-rebaseline.py; since: 2026-08-08)
