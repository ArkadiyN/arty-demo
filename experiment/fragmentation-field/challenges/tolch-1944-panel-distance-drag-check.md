# Tolch (1944) 75mm M48 panel tests — is this an independent check on the drag gap?
...
Gurney)$, 951.0$ (Ordnance-stated)$\}$ m/s.
...
|            1.2 (SAND lo) |                  7.3 | 1.000 .884 .711 .557 | −0.102, −0.081   |
|            1.7 (SAND hi) |                  1.1 | 1.000 .887 .715 .557 | −0.100, −0.077   |
...
corroborated by the independent pit test (Summary items 1 and 8: ~803
fragments recovered per shell, 95.6% of the metal by mass, and "practically
...
|            1.2 (SAND lo) |      7 902 |      9 122 |     14 552 |
|            1.7 (SAND hi) |      9 755 |     11 067 |     16 825 |
...
combined drag to the SAND92-0243 range of 1.2–1.7 inflates the perforating-
fragment count to **11–14×** the measured value. Raising it 5× (the factor the
75mm Ordnance check's implied-λ gap suggested) gives ~15×.
...
- Tolch pit test: **803** fragments carrying 95.6% of the metal → mean
    recovered fragment mass **6.85 g**.
- Matching Tolch's 95.6%-of-mass recovery in the model requires a cut near
    0.13 g, above which the model holds ≈**6 000** fragments — **~7.5× more**
    than the 803 Tolch recovered.
...
1. **Tolch does contradict the proposed *remedy*.** The
    ...
    threshold. Do not adopt a constant drag increase in that range on the
    strength of the Ordnance checks alone. This strengthens that check's own
    closing sentence — the residual is not a constant-drag-scaling error.
1. **Tolch redirects the investigation to the Mott mass spectrum.** The
    ~7.5× fragment-count excess against Tolch's pit test is a larger, cleaner,
    and independently-measured defect than the drag gap, in a different aspect,
    and it plausibly drives much of the 7–33× B(r) over-prediction that started
    this line of work. Recommend scoping the Mott $\mu$ / `mott_params`
    calibration next, **before** any further drag tuning — the two are
