...
$$m_j = \sum_{L_j} m_i \tag{8}$$

$$V_j = \frac{\displaystyle\sum_{L_j} V_i m_i}{m_j} \tag{9}$$

$$r_j = \frac{\displaystyle\sum_{L_j} R_i m_i}{m_j} \cdot \frac{1}{\sin \Theta_j} \tag{10}$$
...
$$N(m) = \sum_j N_{0j} \, e^{-(m/\mu_j)^{1/2}} \tag{18}$$
...
$$N(m) = \sum_k \sum_j N_{0kj} \, e^{-(m/\mu_{kj})^{1/2}} \tag{19}$$
...
$$N_{kj}(m) = N_{0kj} \, e^{-(m/\mu_{kj})^{1/2}} \tag{20}$$

$$\mu_{kj} = \sqrt{\frac{2}{\rho}} \left(\frac{\sigma_{Fk}}{\gamma_k}\right)^{3/2} \left(\frac{r_j}{V_j}\right)^3 \tag{21}$$

$$N_{0kj} = \frac{m_{kj}}{\mu_{kj}} \tag{22}$$

In equations (20–22), $\gamma_k$ and $\sigma_{Fk}$ denote parameters $\gamma$ and $\sigma_F$ in each region $k$, respectively. Hence, because the region material definitions $k$ are tracked in the Lagrangian frame of reference, the one-region model formulation allows varying values of $\gamma_k$ and $\sigma_{Fk}$ through the fragmenting shell regions considered.
...
$$R_k = \frac{\displaystyle\sum_{i}^{L_k} \hat{r}_i A_i}{\displaystyle\sum_{i}^{L_k} A_i} \tag{23}$$
...
$$R_k = \frac{\displaystyle\sum_{i}^{L_k} r_i}{L_k} \tag{24}$$
...
$$\varepsilon_k = \frac{R_k}{R_{k0}} - 1 \tag{25}$$

In equation (25) $R_{k0}$ is defined as

$$R_{k0} = \left.\frac{\displaystyle\sum_{i}^{L_k} r_i}{L_k}\right|_{t=0} \tag{26}$$
...
**Fig. 7.** An example: variable $\gamma$ modelling. (a) Empirical $\gamma$ versus explosive detonation pressures, $P_{CJ}$. (b) Parameter $\gamma$ is a function of the detonation shock wave incidence angle $\alpha$, and varies along the shell. The steeper angle $\alpha$ is, the higher parameter $\gamma$ is.
...
| Body | Explosive | $P_{CJ}$/GPa | Axial booster | Total number of fragments, $N/N^\*$ |
| ---- | --------- | ------------ | ------------- | ----------------------------------- |
| 1090 | IMX       | 19.0         | No            | 1                                   |
